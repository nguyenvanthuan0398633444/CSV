...
namespace ProjectTeamNET.Service.Implement
{
    public class ManhourReportService : IManhourReportService
    {
        ...
        {
            var tmp = dbContext.UserScreenItems.Where(e => e.Surrogate_key.Contains(userScreenName));
        ...
        public async Task<ManhourReportViewModel> Init()
        {
            ...
            model.Users = userscreenRepository.dbset.Where(e=>e.Screen_url== "ManhourReport").Select(s => new SelectListItem { Value = s.Surrogate_key.Substring(0,18), Text = s.Save_name }).Distinct().ToList();
            model.GroupName = await userscreenRepository.Select<GroupNames>(QueryLoader.GetQuery("ManhourReport", "GroupName"));
            return model;
        }
        ...
        public async Task<List<ManhourReport>> SetManhourReport(ManHourReportSearch data)
        {
            ...
            List<ManhourReport> result = await userscreenRepository.Search<ManhourReport>(query, param);
        ...
        public List<double> ManhourReportMonthly(string group, string user, string theme, DateTime toDate, DateTime fromDate, string workContentCodes, string workContentDetails)
        {
            ...
            var result = new List<Double>();
            for(var i = 0; i <= Common.GetMonthBetWeenTwoDate(fromDate, toDate); i++)
            {
                var check = true;
